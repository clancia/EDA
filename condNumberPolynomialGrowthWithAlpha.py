...
for j in range(len(alphas)):
	...
	condNo[j] = np.linalg.cond(A, 1)

plt.figure()
plt.plot(alphas, condNo, 'r-', label='rho=%.2f, q=%.2f' % (r, q))

xd,yd = np.log(alphas[1:]), np.log(condNo[1:])
slope, intercept = np.polyfit(xd, yd, 1)
yfit = np.e**( slope*xd + intercept )
plt.plot(alphas[1:], yfit, 'g:', label='e^[%.3f*log(alpha) + %.2f]' % (slope, intercept))
